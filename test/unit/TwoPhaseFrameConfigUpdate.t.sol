...
contract TwoPhaseFrameConfigUpdateTest is Test {
    ...
    function calculateExpectedSlot(
        ...
        return fromRefSlot + (reportsToProcess * epochsPerFrame * SLOTS_PER_EPOCH);
    }

    function calculateDeadlineSlot(uint256 expectedSlot, uint256 epochsPerFrame) internal pure returns (uint256) {
    ...
    function calculateExpirationSlot(
        ...
        uint256 minEpochs = epochsPerFrame1 < epochsPerFrame2 ? epochsPerFrame1 : epochsPerFrame2;
    ...
    function mockLastProcessingRefSlot(uint256 lastProcessingRefSlot) internal {
        ...
        mockConsensus.setCurrentFrame(0, lastProcessingRefSlot, lastProcessingRefSlot + 100);
    }

    function createUpdater(TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig) internal {
        updater = new TwoPhaseFrameConfigUpdate(address(mockOracle), phasesConfig);
    }
    ...
    function test_constructor_Success() public {
        ...
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(
        ...
        (uint256 restoreExpectedProcessingRefSlot, , , , ) = updater.restorePhase();
    ...
    function test_constructor_RevertWhen_ZeroOracleAddress() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 2, 1, 64);

        vm.expectRevert(TwoPhaseFrameConfigUpdate.ZeroOracleAddress.selector);
        new TwoPhaseFrameConfigUpdate(address(0), phasesConfig);
    }

    function test_constructor_RevertWhen_ZeroReportsToProcessBeforeOffsetPhase() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(0, 2, 1, 64); // Zero reportsToProcess for offsetPhase

        uint256 testDay = 10;
        mockLastProcessingRefSlot(epochEndSlot(testDay));

        vm.expectRevert(TwoPhaseFrameConfigUpdate.ZeroReportsToEnableUpdate.selector);
        new TwoPhaseFrameConfigUpdate(address(mockOracle), phasesConfig);
    }

    function test_constructor_RevertWhen_ZeroReportsToProcessBeforeRestorePhase() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 0, 1, 64); // Zero reportsToProcess for restorePhase
    ...
    function test_constructor_RevertWhen_ZeroEpochsPerFrame() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 2, 0, 64); // Zero epochsPerFrame for offsetPhase
    ...
    function test_constructor_RevertWhen_CurrentReportMainPhaseIsNotCompleted() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 2, 1, 64);
        ...
        mockOracle.setLastProcessingStartedRefSlot(lastProcessingRefSlot);
        mockConsensus.setCurrentFrame(0, differentCurrentRefSlot, differentCurrentRefSlot + 100);

        vm.expectRevert(TwoPhaseFrameConfigUpdate.CurrentReportMainPhaseIsNotCompleted.selector);
        new TwoPhaseFrameConfigUpdate(address(mockOracle), phasesConfig);
    }
    ...
    function test_constructor_RevertWhen_FastLaneTooShort() public {
        ...
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 2, 1, 63); // 63 < 64 (2 * 32)
    ...
    function test_constructor_RevertWhen_FastLanePeriodTooLong() public {
        ...
        vm.expectRevert(TwoPhaseFrameConfigUpdate.FastLanePeriodCannotBeLongerThanFrame.selector);
        new TwoPhaseFrameConfigUpdate(address(mockOracle), phasesConfig);
    }

    function test_constructor_DefaultsRestorePhaseConfig() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 3, 2, 64); // only reportsToProcess provided
        ...
        uint256 offsetExpectedProcessingRefSlot = fromRefSlot +
            (phasesConfig.reportsToProcessBeforeOffsetPhase * DEFAULT_EPOCHS_PER_FRAME * SLOTS_PER_EPOCH);
        ...
        uint256 minEpochs = currentEpochsPerFrame < phasesConfig.offsetPhaseEpochsPerFrame
            ? currentEpochsPerFrame
            : phasesConfig.offsetPhaseEpochsPerFrame;
        uint256 expectedRestorePhaseExpiration = expectedRestorePhaseProcessingRefSlot + (minEpochs * SLOTS_PER_EPOCH);
    ...
    function test_constructor_RevertWhen_OffsetPhaseAlreadyExpired() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 2, 1, 64); // 1 report, 1 day, 10 fast lane slots
    ...
    function test_executeOffsetPhase_Success() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 2, 1, 64); // 1 report, 1 day, 10 fast lane slots, restorePhase defaults
    ...
    function test_executeOffsetPhase_RevertWhen_UnexpectedLastProcessingRefSlot() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 2, 1, 64); // 1 report, 1 day, 10 fast lane slots
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                TwoPhaseFrameConfigUpdate.UnexpectedLastProcessingRefSlot.selector,
                wrongSlot,
    ...
    function test_executeOffsetPhase_RevertWhen_AlreadyExecuted() public {
        ...
        vm.expectRevert(TwoPhaseFrameConfigUpdate.PhaseAlreadyExecuted.selector);
        updater.executeOffsetPhase();
    }
    ...
    function test_executeOffsetPhase_RevertWhen_DeadlineExpired() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TwoPhaseFrameConfigUpdate.PhaseExpired.selector, (deadlineSlot + 1), deadlineSlot)
        );
        updater.executeOffsetPhase();
    }

    function test_executeRestorePhase_Success() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 2, 1, 64); // defaults: current frame & offsetPhase fast lane
        ...
        mockConsensus.grantRole(mockConsensus.MANAGE_FRAME_CONFIG_ROLE(), address(updater));

        // Execute Offset phase
        uint256 offsetExpectedSlot = calculateExpectedSlot(fromRefSlot, 1, DEFAULT_EPOCHS_PER_FRAME);
        mockOracle.setLastProcessingStartedRefSlot(offsetExpectedSlot);
        updater.executeOffsetPhase();

        // Execute Restore phase
        uint256 restoreExpectedSlot = calculateExpectedSlot(offsetExpectedSlot, 2, EPOCHS_PER_DAY);
        ...
        assertEq(mockConsensus.lastSetEpochsPerFrame(), DEFAULT_EPOCHS_PER_FRAME);
        assertEq(mockConsensus.lastSetFastLaneLengthSlots(), 64);

        // Verify role was renounced
        assertFalse(mockConsensus.hasRole(mockConsensus.MANAGE_FRAME_CONFIG_ROLE(), address(updater)));
    ...
    function test_executeRestorePhase_RevertWhen_WithoutOffsetPhase() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 2, 1, 64); // restorePhase uses defaults
        ...
        vm.expectRevert(TwoPhaseFrameConfigUpdate.OffsetPhaseNotExecuted.selector);
        updater.executeRestorePhase();
    }

    function test_executeRestorePhase_RevertWhen_UnexpectedLastProcessingRefSlot() public {
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 2, 1, 64); // restorePhase uses defaults
    ...
    function test_executeRestorePhase_RevertWhen_WithDeadlineExpired() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TwoPhaseFrameConfigUpdate.PhaseExpired.selector, currentSlot, restoreExpirationSlot)
        );
        updater.executeRestorePhase();
    }
    ...
    function test_renounceRoleWhenExpired_WhenOffsetPhaseExpired() public {
        ...
        mockConsensus.grantRole(mockConsensus.MANAGE_FRAME_CONFIG_ROLE(), address(updater));
        assertTrue(mockConsensus.hasRole(mockConsensus.MANAGE_FRAME_CONFIG_ROLE(), address(updater)));
    ...
    function test_renounceRoleWhenExpired_WhenNoPhasesExpired() public {
        ...
        (bool offsetExpired, bool restoreExpired) = updater.getExpirationStatus();
    ...
    function test_phaseStatesInitialized() public {
        ...
        uint256 expectedOffsetPhaseSlot = calculateExpectedSlot(fromRefSlot, 1, DEFAULT_EPOCHS_PER_FRAME);
        uint256 expectedRestorePhaseSlot = calculateExpectedSlot(expectedOffsetPhaseSlot, 2, EPOCHS_PER_DAY);
        ...
        assertEq(restoreFastLaneLengthSlots, phasesConfig.restorePhaseFastLaneLengthSlots);
    ...
    function test_expirationSlotCalculation() public {
        ...
        TwoPhaseFrameConfigUpdate.PhasesConfig memory phasesConfig = createPhasesConfig(1, 1, 1, 64); // defaults
        ...
        uint256 expectedOffsetPhaseExpiration = offsetExpectedSlot + (dayToEpochs(1) * SLOTS_PER_EPOCH);
        assertEq(
        ...
        uint256 restoreExpectedSlot = calculateExpectedSlot(offsetExpectedSlot, 1, dayToEpochs(1));
        // Restore phase expiration should use min(dayToEpochs(1)=225, DEFAULT=2925) = 225
        uint256 expectedRestorePhaseExpiration = restoreExpectedSlot + (dayToEpochs(1) * SLOTS_PER_EPOCH);
        assertEq(
